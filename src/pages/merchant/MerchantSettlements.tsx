    ...
    <div className="space-y-4">
      ...
      <Card className="p-4 text-sm">
        ...
        <ol className="mt-1 list-decimal space-y-0.5 pl-5 text-muted-foreground">
          ...
          <li>Gross sales − commission − delivery fee − service fee − processing fee = your net payout.</li>
          <li>Once admin marks it Paid, the funds are transferred to your registered bank account.</li>
        </ol>
      </Card>
      <Card className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="border-b bg-muted/40"><tr className="text-left"><th className="p-3">Period</th><th className="p-3">Gross</th><th className="p-3">Commission</th><th className="p-3">Delivery</th><th className="p-3">Service</th><th className="p-3">Processing</th><th className="p-3">Net payout</th><th className="p-3">Status</th><th className="p-3">Paid</th></tr></thead>
          <tbody>
            {items.map((s) => (
              <tr key={s.id} className="border-b"><td className="p-3">{s.period_start} → {s.period_end}</td><td className="p-3">{fmt(s.gross_sales)}</td><td className="p-3">{fmt(s.commission_amount)}</td><td className="p-3">{fmt(s.delivery_fee_total)}</td><td className="p-3">{fmt(s.service_fee_total)}</td><td className="p-3">{fmt(s.processing_fee_total)}</td><td className="p-3 font-bold">{fmt(s.net_payout)}</td><td className="p-3 capitalize">{s.status}</td><td className="p-3 text-xs">{s.paid_at ? new Date(s.paid_at).toLocaleDateString() : "—"}</td></tr>
            ))}
            {items.length === 0 && <tr><td colSpan={9} className="p-6 text-center text-sm text-muted-foreground">No settlements yet.</td></tr>}
          </tbody>
        </table>
      </Card>
    </div>
