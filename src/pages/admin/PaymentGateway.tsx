    ...
    <Card className="p-6 space-y-5 max-w-3xl">
      ...
      <div className="rounded-md border bg-muted/40 p-3 text-xs">
        ...
        <code className="break-all">
          {import.meta.env.VITE_SUPABASE_URL}/functions/v1/{webhookFunction}
        </code>
        <p className="mt-1 text-muted-foreground">{webhookHint}</p>
      </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Configure payment providers used for collecting and sending money.
        </p>
      </div>

      <Tabs defaultValue="collect" className="max-w-3xl">
        <TabsList>
        ...
        <TabsContent value="collect" className="mt-4">
          <GatewayForm
            provider="chip"
            title="Chip-in (CHIP) — Collect"
            description="Terima duit daripada pelanggan — FPX (Online Transfer), Credit Cards, e-wallets."
            icon={<CreditCard className="h-5 w-5 text-primary" />}
            testFunction="chip-test-connection"
            webhookFunction="chip-webhook"
            webhookHint="Add this as the success callback in your CHIP brand settings."
            brandLabel="Brand ID"
          />
        </TabsContent>
        ...
        <TabsContent value="send" className="mt-4">
          <GatewayForm
            provider="chip_send"
            title="Chip-in (CHIP) — Send"
            description="Hantar duit (disbursement / payout) ke akaun bank merchant dan rider."
            icon={<Send className="h-5 w-5 text-primary" />}
            testFunction="chip-test-send-connection"
